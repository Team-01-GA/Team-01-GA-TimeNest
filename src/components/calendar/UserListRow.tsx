...
function UserListRow({ handle }: { handle: string }) {
    ...
    return (
        <div onClick={(e) => handleGoToAccount(e)} className="flex flex-row gap-3 w-full p-1 items-center cursor-pointer rounded-box border border-primary-content transition-all hover:bg-secondary hover:border-transparent group">
            {profileImg 
                ? <img className="w-8 h-8 rounded-[50%]" src={profileImg} alt="Profile picture"/>
                : <div className="flex flex-row justify-center items-center w-8 h-8 rounded-[50%] bg-primary">

...
import { createUserObject, getUserByEmail, getUserByHandle, getUserByPhone } from '../../services/users.service';
...
function Register({ loading, setLoading }: { loading: boolean; setLoading: React.Dispatch<React.SetStateAction<boolean>>; }) {
            ...
            <fieldset className="fieldset bg-base-200 border-base-300 rounded-box w-full border p-4 gap-4">
                <div className='flex flex-row gap-2 w-full'>
                    <label className="floating-label w-1/2">
                        ...
                        <input disabled={loading} className="input input-lg" name='given-name' placeholder='John' type="text" autoComplete='given-name' value={fields.firstName} onChange={e => saveInputs('firstName', e.target.value)}/>
                    </label>
                    <label className="floating-label w-1/2">
                        <span>Last name</span>
                        <input disabled={loading} className="input input-lg" name='family-name' placeholder='Doe' type="text" autoComplete='family-name' value={fields.lastName} onChange={e => saveInputs('lastName', e.target.value)}/>
                    </label>
                </div>
                <label className="floating-label">
                    <span>Username</span>
                    <input disabled={loading} className="input input-lg w-full" name='username' placeholder='johnthedoe123' type="text" autoComplete='username' value={fields.handle} onChange={e => saveInputs('handle', e.target.value)}/>
                </label>
                <label className="floating-label">
                    <span>Email</span>
                    <input disabled={loading} className="input input-lg w-full" name='email' placeholder='johndoe@timenest.com' type="email" autoComplete='email' value={fields.email} onChange={e => saveInputs('email', e.target.value)}/>
                </label>
                <label className="floating-label">
                    <span>Phone number</span>
                    <input disabled={loading} className="input input-lg w-full" name='tel' placeholder='0888888888' type="tel" autoComplete='tel' value={fields.number} onChange={e => saveInputs('number', e.target.value)}/>
                </label>
                <label className="floating-label">
                    <span>Password</span>
                    <input disabled={loading} className="input input-lg w-full" name='new-password' placeholder='1234567' type="password" autoComplete='new-password' value={fields.password} onChange={e => saveInputs('password', e.target.value)}/>
                </label>
                <button disabled={loading} className='btn btn-primary btn-lg mt-4 relative' type='submit'>
                    Sign up
                </button>
            </fieldset>
        </form>
